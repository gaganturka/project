...
const FirmPracticeAreas = require("../../models/FirmPracticeAreas");
...
const index = async (req, res) => {
    let models = await FirmPracticeAreas.paginate({
        firmId: req.firm._id,
...
const create = async (req, res) => {
    try {
        ...
        let model = null;
        if (name != null) {
            model = await FirmPracticeAreas.findOne({
                name: name,
                firmId: req.firm.id
            });
        }
        ...
        if (model) {
            throw Boom.badRequest(responseMessages.PRACTICE_AREA_ALREADY_EXIST);
        }

        model = new FirmPracticeAreas();
...
const view = async (req, res) => {
    try {
        const model = await FirmPracticeAreas.findOne({
            _id: req.params.id,
            firmId: req.firm.id
        });
        if (model != null) {
...
const update = async (req, res) => {
    try {
        ...
        if (model != null) {
            ...
            if (name != null) {
                let modelWithSame = await FirmPracticeAreas.findOne({
                    name: name,
                    firmId: req.firm.id,
                    _id: {$ne: model._id}
                });
