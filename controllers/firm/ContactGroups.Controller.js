...
const create = async (req, res) => {
    try {
        ...
        let model = null;
        if (name != null) {
            model = await FirmContactGroups.findOne({
                name: name,
                firmId: req.firm.id
            });
        }
        ...
        if (model) {
            throw Boom.badRequest(responseMessages.CONTACT_GROUP_ALREADY_EXIST);
        }

        model = new FirmContactGroups();
...
const view = async (req, res) => {
    try {
        const model = await FirmContactGroups.findOne({
            _id: req.params.id,
            firmId: req.firm.id
        });
        if (model != null) {
...
const update = async (req, res) => {
    try {
        ...
        if (model != null) {
            ...
            if (name != null) {
                let modelWithSame = await FirmContactGroups.findOne({
                    name: name,
                    firmId: req.firm.id,
                    _id: {$ne: model._id}
                });
