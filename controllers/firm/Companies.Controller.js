...
const FirmCompanies = require("../../models/FirmCompanies");
...
const index = async (req, res) => {
    let models = await FirmCompanies.paginate({
        firmId: req.firm._id,
...
const create = async (req, res) => {
    try {
        ...
        let model = null;
        if (name != null) {
            model = await FirmCompanies.findOne({
                name: name,
                firmId: req.firm.id
            });
        }
        ...
        if (model) {
            throw Boom.badRequest(responseMessages.COMPANY_ALREADY_EXIST);
        }

        model = new FirmCompanies();
...
const view = async (req, res) => {
    try {
        const model = await FirmCompanies.findOne({
            _id: req.params.id,
            firmId: req.firm.id
        });
        if (model != null) {
            ...
            throw new Error(responseMessages.FIRM_COMPANY_NOT_FOUND);
        }
    } catch (err) {
...
const update = async (req, res) => {
    try {
        ...
        if (model != null) {
            ...
            if (name != null) {
                let modelWithSame = await FirmCompanies.findOne({
                    name: name,
                    firmId: req.firm.id,
                    _id: {$ne: model._id}
                });
