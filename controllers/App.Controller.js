...
module.exports = {
    ...
    getExpertDetails: async (req, res) => {
        try {
            ...
            //prefill user available limits as per price
            if (expert.priceDetails && expert.priceDetails.length > 0) {
                for (let priceType in priceTypes) {
                    priceTypes[priceType] = await expert.priceDetailsForUser(priceType, req.user.id);
                }
            }
